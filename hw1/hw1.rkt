...
(define (what-beats x)
  (cond
   [(equal? x "rock") (string->immutable-string "paper")]
   [(equal? x "paper") (string->immutable-string "scissors")]
   [(equal? x "scissors") (string->immutable-string "rock")]
   [else (string->immutable-string "invalid")]
   ))

(define (result x y)
  (cond
   [(equal? (what-beats x) y) (string->immutable-string "lose")]
   [(equal? x (what-beats y)) (string->immutable-string "win")]
   [(equal? x y) (string->immutable-string "tie")]
   [else (string->immutable-string "invalid")]
   ))
...
(define (digit-sum n)
    (if (equal? #f (= 0 (quotient n 10)))
           (+ (remainder n 10) (digit-sum (quotient n 10))) (remainder n 10)))

(define (reduce n)
  (define x (digit-sum n))
  (if (> x 9) (reduce x) x))
...
(define (top-count exp lst)
  (cond
   [(empty? lst) 0]
   [else
      (if (equal? exp (first lst))
          (+ 1 (top-count exp (rest lst)))
          (top-count exp (rest lst)))]))
; ********************************************************
; ** problem 5 ** (11 points)
; Write a procedure
...
(define (diff-sequence lst)
  (cond
   ...
   [else
      (cons (- (first (rest lst)) (first lst)) 
          (diff-sequence (rest lst)))]))
...
(define (skip-every-other lst)
  (cond
   ...
   [else 
       (cons
        (first lst)
        (if (equal? (empty? (rest lst)) #f) (skip-every-other (rest (rest lst))) empty))]))
